...
- `| jq '.'`：再使用管道符 `|` 将 `echo ...` 的输出结果传到给 `jq` 处理器。`jq` 是一个强大的命令行 JSON 处理器，`'.'` 是一个简单的 jq 表达式，表示`原样格式化输出接收的JSON数据`。 
...
2
```
- `| jq '.[] | length'`：其中 `'.[]'` 表示`遍历数组中的每个元素`；`length` 表示`返回每个元素的长度`；

  `[{"name": "JSON", "good": true}, {"name": "XML", "good": false}]` 这一JSON数组包含了两个对象元素，每个对象都有两个属性，分别是`name` 和 `good`，所以输出为2（`每个对象的属性数量`）
