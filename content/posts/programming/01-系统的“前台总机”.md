+++
title = '第一章 系统的“前台总机”'
date = 2024-12-02T23:00:30+08:00
draft = false
...
我们不能让大厅（`main`函数）变成菜市场。所以，我们来成立一个“用户部”（User Department），在代码里，它叫 `UserHandler`，我打算在其之上定义用户（User）相关的路由，也就是用户相关业务的请到“人才部门”办理。
...
- internal包：这个目录只处理本项目内部的一些业务，比如你引用其他第三方依赖，但你仍然无法使用它internal里面定义的内容，即使这些内容是可导出的，可以认为它是内部独有的，相当于公司的内部部门，不会和外部对接。
...
上面方案B已经很不错了，但我个人还有个更“懒”的玩法，我称之为“部门自治”。我不设一个中央的“路由登记处”，而是**让每个部门自己负责登记自己的路由**。
...
func (p *ProductHandler) RegisterRoutes(server *gin.Engine) {
	pg := server.Group("/products")
	{
		pg.POST("/list", u.List)
		...
	}
...
- 为了路由分组
- 很重要，为了避免使用包变量，方面之后的依赖注入，从而让代码具有很好的测试性。
