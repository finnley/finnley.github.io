+++
title = '第二章 严格的“审查员”'
date = 2024-12-04T23:00:30+08:00
draft = true
...
再看 `json:"email"` 这部分，这像是在登记表上做的“备注”，告诉我们的智能助理 `Gin`：“嘿，如果信封里的表格上写着 `email` 的栏位，就把它的内容抄到我这张表的 `Email` 栏里来！”。也就是前端传的json数据字段是“email”而不是“Email”，即这个“Email”字段在json中对应的是“email”。

## 2 用“魔法扫描仪”自动填表 (`ctx.Bind`)

好了，空白表格有了，信封也来了。难道要我们的专员一个字一个字地手动抄写吗？不，我们有高科技——`ctx.Bind()`，一台能自动识别 `JSON` 格式的魔法扫描仪！

```go
// internal/web/user.go -> SignUp 方法内
func (u *UserHandler) SignUp(ctx *gin.Context) {
	type SignUpReq struct {
		Email           string `json:"email"`
		Password        string `json:"password"`
