...
Docker 实现进程的隔离本质上通过 clone 这个方法来实现的
...
- **UTS namespace**  
  UTS(UNIX Time-sharing System)namespace 提供了主机名与域名的隔离，这样每个 docker 容器就可以拥有独立的主机名和域名了，在网络上可以被视为一个独立的节点，而非宿主机上的一个进程。

- **IPC namespace**  
  进程间通信(Inter-Process Communication, IPC)涉及的 IPC 资源包括常见的信号量、消息队列和共享内存。在同一个 IPC namespace 下的进程彼此可见，不同 IPC namespace 下的进程则互相不可见。

- **PID namespace**  
  PID namespace 隔离非常实用，它对进程 PID 重新标号，即两个不同 namespace 下的进程可以有相同的 PID。每个 PID namespace 都有自己的计数程序。内核为所有的 PID namespace 维护了一个树状结构，最顶层的是系统初始时创建的，被称为 root namespace，它创建的子 PID namespace 被称为 child namespace（树的子节点）。  
  通过这种方式，不同的 PID namespace 会形成一个层级体系。所属的父节点可以看到子节点中的进程，并可以通过信号等方式对子节点中的进程产生影响。反过来，子节点却不能看到父节点 PID namespace 中的任何内容。

---

图片完整内容识别并整理如下（已校对、补全并解释清楚）：

### 实践：通过 unshare() 在原先进程上进行 namespace 隔离

最主要说明的系统调用是 **unshare()**，它与 clone() 很像，**不同的是，unshare() 运行在原先的进程上，不需要启动一个新进程**。
...
命令解释：
- `unshare`：执行 namespace 隔离
- `--fork`：在隔离后再 fork 一个子进程（通常必须配合使用，否则直接在当前 shell 上隔离会导致整个终端异常）
- `--pid`：创建新的 PID namespace
- `--mount-proc`：在新的 PID namespace 中重新挂载 /proc 文件系统（非常重要！）
- `bash`：启动一个新的 shell 进入隔离后的环境

执行完这行命令后，你会进入一个全新的 bash，此时你已经处于一个**独立的 PID namespace** 中，在这个进程空间中只有两个进程。
...
```bash
[root@vm-1 opt]# ps aux
USER       PID %CPU %MEM    VSZ   RSS TTY      STAT START   TIME COMMAND
root         1  0.2  0.0   7636  4556 ?        Ss   20:14   0:00 /usr/sbin/init
root        20  0.0  0.0  13168  4680 ?        Ss   20:14   0:00 /usr/lib/systemd/systemd-journald
root        31  0.0  0.0  80540  2624 ?        Ss   20:14   0:00 /usr/sbin/lvmetad -f
root        33  0.1  0.0   5404  2832 ?        Ss   20:14   0:00 /usr/lib/systemd/systemd-udevd
root        87  0.0  0.0   4260  2356 ?        Ss   20:14   0:00 /usr/lib/systemd/systemd-logind
dbus        89  0.0  0.0   8204  3500 ?        Ss   20:14   0:00 /usr/bin/dbus-daemon --system --address=systemd: --nofork --nopidfile --systemd-activation
root        91  0.0  0.4 1540512 39216 ?       Ssl  20:14   0:00 /usr/bin/containerd
root       101  0.0  0.0   3980  1320 console  Ss+  20:14   0:00 /sbin/agetty --noclear --keep-baud console 115200,38400,9600 vt220
root       106  0.2  0.9 1719460 77340 ?       Ssl  20:14   0:00 /usr/bin/dockerd -H fd:// --containerd=/run/containerd/containerd.sock
root       148  0.0  0.0  17620  6840 ?        Ss   20:14   0:00 /usr/sbin/sshd -D
root       253  0.0  0.0   5548  2784 pts/1    Ss+  20:15   0:00 bash
root       274  0.0  0.0   5548  2960 pts/2    Ss   20:15   0:00 bash
root       290  0.0  0.0   9620  3284 pts/2    R+   20:15   0:00 ps aux
[root@vm-1 opt]# sudo unshare --fork --pid --mount-proc bash
[root@vm-1 opt]# ps aux
USER       PID %CPU %MEM    VSZ   RSS TTY      STAT START   TIME COMMAND
root         1  0.0  0.0   5548  2952 pts/2    S    20:16   0:00 bash
root        12  0.0  0.0   9620  3236 pts/2    R+   20:16   0:00 ps aux
[root@vm-1 opt]#
```

**现象解释**：
- 当前 shell 的 bash 进程变成了 PID = 1（在新的 PID namespace 中，它是“init进程）
- 整个系统中你只能看到当前 namespace 里的进程（通常就只有 bash 和 ps 两个）
- 宿主机上原本成百上千的进程全部“消失”了
- 这正是 PID namespace 隔离的直观体现！

### 小结
- `clone()`：创建新进程时顺便隔离 namespace（Docker 底层常用方式）
- `unshare()`：不创建新进程，直接在当前进程上“现场”隔离 namespace（适合调试和临时实验）

通过上面这个 `unshare --fork --pid --mount-proc bash` 命令，你就能亲手体验一个“亲眼看到”PID namespace 隔离的效果，非常直观推荐大家实际操作一遍。

这与我们进入docker容器中的现象类似：
```bash
