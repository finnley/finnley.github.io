...
**什么是 key-value?** 
类似于 map 或者 dict 字典的功能，可以通过一个 `key` 查找到 一个对应的 `value`。
...
上图是客户端从请求到Redis的完整生命周期，可以简化成4个过程

1. 发送一个命令，比如 set "hello world" 的命令，它是通过网络进行发送的；
2. Redis是单线程的，可以想象成一个队列，在命令执行之前都是在排队的；
3. 命令执行；
4. 将执行结果返回给客户端；

慢查询发生在第三阶段，假如命令属于慢查询的范围，命令真正执行了消耗的时间，而不是它排队，或者网络发送，通过网络返回结果花费的时间；
客户端超时不一定慢查询，但慢查询是客户端少超时的一个可能因素

## 两个配置

1. slowlog-max-len

慢查询是一个先进先出的队列，加入一条命令在第三步过程中，它被列为慢查询的范围内，就会进入一个队列，其实这就是利用Redis的列表实现的，是个先进先出的队列；
这个队列是个固定长度的，当队列满了之后，最先进入队列的就会第一个踢出去；
慢查询保存在内存当中，当Redis重启之后不会持久化，会随重启而消失，结构如下:

![](/images/redis/62.png)

slowlog-log-slower-than=1000: 第一个配置是超时，比如超过10ms就进入慢查询范围内，就会把它加入到队列中；
slowlog100: 长度配置，就是固定长度是多少，这里是100；
